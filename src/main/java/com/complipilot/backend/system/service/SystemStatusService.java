...
public class SystemStatusService {
        ...
        public SystemStatusResponse getStatus() {
                ...
                String overallStatus = components.stream()
                                .anyMatch(component -> STATUS_DOWN.equals(component.status()))
                                                ? STATUS_DOWN
                                                : components.stream().anyMatch(
                                                                component -> STATUS_WARN.equals(component.status()))
                                                                                ? STATUS_WARN
                                                                                : STATUS_UP;
        ...
        private SystemStatusComponentResponse mailComponent() {
                ...
                boolean configured = !enabled || (mailProperties.isResend()
        ...
        private SystemStatusComponentResponse billingComponent() {
                boolean apiKeyConfigured = hasText(stripeProperties.getApiKey());
                boolean webhookSecretConfigured = hasText(stripeProperties.getWebhookSecret());
                boolean proConfigured = hasText(stripeProperties.getProPriceId());
                boolean businessConfigured = hasText(stripeProperties.getBusinessPriceId());

                boolean configured = apiKeyConfigured && webhookSecretConfigured;
                boolean fullyConfigured = configured && proConfigured && businessConfigured;

                return new SystemStatusComponentResponse(
                                "billing",
                                "Billing Service",
                                fullyConfigured ? STATUS_UP : (configured ? STATUS_WARN : STATUS_DOWN),
                                fullyConfigured
                                                ? "Stripe billing is fully configured."
                                                : configured
                                                                ? "Stripe API keys are present, but missing some price IDs."
                                                                : "Stripe API key is not configured.",
                                Map.of(
                                                "provider", "stripe",
                                                "apiKeyConfigured", apiKeyConfigured,
                                                "webhookSecretConfigured", webhookSecretConfigured,
                                                "proPriceIdConfigured", proConfigured,
                                                "businessPriceIdConfigured", businessConfigured));
        }
